...
<sub>[中文文档](#中文) · [↑ back to top](#imagegen-cli)</sub>
...
`imagegen-cli` provides a compact command-line interface for managing several image-generation API profiles and saving generated images directly to a chosen path. It is built for personal use, shell scripts, Agents, and automation workflows.
...
- **Multiple profiles** — Keep several models, endpoints, and API keys in one [configs.json](configs.example.json).
...
The `imagegen` command is then available from any directory.
...
> **Security**<br>
> `configs.json` is ignored by Git. Never commit a file containing a real API key; use [configs.example.json](configs.example.json) when sharing configuration publicly.
...
<sub>[English docs](#english) · [↑ 回到顶部](#imagegen-cli)</sub>
...
- **多用户配置**：在一个 [configs.json](configs.example.json) 中管理多个模型、端点和 API Key。
...
> **安全提示**<br>
> `configs.json` 已被 `.gitignore` 忽略。请勿提交包含真实 API Key 的配置文件；公开分享时请使用 [configs.example.json](configs.example.json)。
